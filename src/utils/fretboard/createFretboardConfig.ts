...
function getDefaultStringWidth(stringIndex: number, stringCount: number) {
  ...
  const thickness = 0.9 + (stringIndex / (stringCount - 1)) * 0.85;
...
export function createFretboardConfig(
  ...
  const themeConfig = fretboardThemes[themeName].config;
  ...
  return {
    ...
    stringWidths: normalizeStringStyleArray(
      ...
      setup.tuning.length,
      (index) => getDefaultStringWidth(index, setup.tuning.length),
    ),
  };
}
